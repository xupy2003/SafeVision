...
def find_sublist_end(main_list, sublist):
    n = len(sublist)
...
class QATrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        outputs = model(pixel_values=pixel_values, input_ids=input_ids, attention_mask=attention_mask, image_flags=image_flags)
        ...
        chat_end_tokens = self.tokenizer.encode("assistant\n", add_special_tokens=False)
        gpt_start_position = find_sublist_end(input_ids[0].tolist(), chat_end_tokens) 
        mask = torch.zeros_like(shift_labels, dtype=torch.float)

        for i in range(mask.size(0)):  
            mask[i, gpt_start_position - 1:] = 1.0  
            if gpt_start_position + 2 < mask.size(1):  
                mask[i, gpt_start_position] = (input_ids.shape[1] - gpt_start_position) / 6
                mask[i, gpt_start_position+1] = (input_ids.shape[1] - gpt_start_position) / 6
                mask[i, gpt_start_position+2] = (input_ids.shape[1] - gpt_start_position) / 6
